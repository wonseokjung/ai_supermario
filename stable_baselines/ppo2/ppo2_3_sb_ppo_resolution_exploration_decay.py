...
class Runner(AbstractEnvRunner):
    ...
    def run(self):
        ...
        for _ in range(self.n_steps):
            ...
            ##exploraion resolution

            epsilon_start = 1.0
            epsilon_final = 0.01
            epsilon_decay = 10000
            epsilon_by_frame = lambda _: epsilon_final + (epsilon_start - epsilon_final) * math.exp(-1. * _ / epsilon_decay)

            resol_epsilon = epsilon_by_frame(_)
            ...
            print(action_omega)
            if random.random() <= resol_epsilon:
                print(resol_epsilon)
                action_omega = random.randint(1,21)
                for i in range(action_omega):
                    self.obs[:], rewards, self.dones, infosinfo = self.env.step(clipped_actions)
                    print("exploration","iteration",i, "action", clipped_actions)
                    mb_rewards.append(rewards)

            else:

                self.obs[:], rewards, self.dones, infosinfo = self.env.step(clipped_actions)
                print("not exploration","iteration",i, "action", clipped_actions)
                mb_rewards.append(rewards)
