    ...
    def learn(self, total_timesteps, callback=None, seed=None, log_interval=100, tb_log_name="DQN"):
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name) as writer:
            ...
            for step in range(total_timesteps):
                ...
                env_action = action
                reset = False
                action_omega = random.randint(1,11)
                print("action omega is", action_omega)
                epsilon_start = 1.0
                epsilon_final = 0.01
                epsilon_decay = 5000
                epsilon_by_frame = lambda step: epsilon_final + (epsilon_start - epsilon_final) * math.exp(-1. * step / epsilon_decay)
                resol_epsilon = epsilon_by_frame(step)
